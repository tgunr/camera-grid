...
class MaskApp:
    ...
    def _build_ui(self) -> None:
        ...
        # ── Lens optics panel ──────────────────────────────────────────────
        self._lens_frame = Frame(left, padx=2, pady=6, relief="groove", bd=1)
        self._lens_frame.pack(fill="x", pady=(8, 0))
        Label(self._lens_frame, text="Lens / Optics (auto optimum)",
              font=("Helvetica", 10, "bold")).pack(anchor="w", padx=6, pady=(4, 2))

        if not HAS_LENS_OPTICS:
            Label(self._lens_frame, text="lens_optics.py not found — panel disabled.",
                  foreground="#a33").pack(anchor="w", padx=6, pady=2)
        else:
            top = Frame(self._lens_frame)
            top.pack(fill="x", padx=4, pady=2)

            # Preset selector
            preset_names = ["—"] + sorted(LENS_PRESETS.keys())
            Label(top, text="Preset:").pack(side="left", padx=4)
            OptionMenu(top, self.lens_preset_var, *preset_names).pack(side="left", padx=4)
            Checkbutton(top, text="Auto-apply", variable=self.lens_auto_var).pack(side="left", padx=8)

            grid = Frame(self._lens_frame)
            grid.pack(fill="x", padx=4, pady=2)

            def add_lens_row(label: str, var, row: int, width: int = 7):
                Label(grid, text=label, anchor="w", width=18).grid(row=row, column=0, sticky="w", padx=4, pady=1)
                e = Entry(grid, width=width, justify="right", textvariable=var)
                e.grid(row=row, column=1, padx=4, pady=1)
                return e

            add_lens_row("Sensor width (mm)", self.lens_sensor_w_var, 0)
            add_lens_row("Focal length (mm)", self.lens_focal_var, 1)
            Label(grid, text="0 = full zoom range", foreground="#666", font=("Helvetica", 8)).grid(row=1, column=2, sticky="w")
            add_lens_row("f-number", self.lens_fnum_var, 2)
            Label(grid, text="0 = preset f/#", foreground="#666", font=("Helvetica", 8)).grid(row=2, column=2, sticky="w")
            add_lens_row("Resolution W (px)", self.lens_res_w_var, 3)
            add_lens_row("Min wall (mm)", self.lens_wall_var, 4)
    ...
    def _on_lens_preset_change(self):
        if not HAS_LENS_OPTICS:
            return
        name = self.lens_preset_var.get()
        if name in LENS_PRESETS:
            preset = LENS_PRESETS[name]
            # Populate sensor/resolution from preset; do not overwrite focal if user set custom
    ...
    def _schedule_lens_recompute(self):
        if not HAS_LENS_OPTICS:
            return
        if self.lens_auto_var.get():
            self.root.after(150, self._on_lens_compute)

    def _collect_lens_criteria(self):
        """Return (focals, fnums, sensor_w, res_w, wall, ghost, stagger) or None if incomplete."""
        if not HAS_LENS_OPTICS:
            return None
        try:
            wall = float(self.lens_wall_var.get())
            ghost = float(self.lens_ghost_var.get())
            res_w = int(float(self.lens_res_w_var.get()))
            sensor_w = float(self.lens_sensor_w_var.get())
            stagger = bool(self.stagger_var.get())
        except (ValueError, TclError):
            return None
        if sensor_w <= 0 or res_w <= 0 or wall <= 0:
            return None
        preset_name = self.lens_preset_var.get()
        focal = float(self.lens_focal_var.get() or 0)
        fnum = float(self.lens_fnum_var.get() or 0)
        if preset_name in LENS_PRESETS and focal == 0:
            # Use full zoom range from preset
            preset = LENS_PRESETS[preset_name]
            focals = list(preset["focal_lengths_mm"])
            fnums = list(preset["f_numbers"])
            return focals, fnums, sensor_w, res_w, wall, ghost, stagger
        if focal > 0 and fnum > 0:
            return [focal], [fnum], sensor_w, res_w, wall, ghost, stagger
        if preset_name in LENS_PRESETS and preset_name != "—":
            preset = LENS_PRESETS[preset_name]
            focals = list(preset["focal_lengths_mm"])
            fnums = list(preset["f_numbers"])
            return focals, fnums, sensor_w, res_w, wall, ghost, stagger
    ...
    def _on_lens_compute(self, apply_override: bool = False):
        ...
        try:
            result = compute_fine_grid_optimum(
                focals, fnums, sensor_w, res_w, wall, ghost, stagger=stagger,
            )
        except Exception as exc:
            self._lens_info_text.set(f"Optics error: {exc}")
            return

        # Convert optimum to current display unit
        unit = self.unit.get()
        if unit == "mm":
            hole_disp = result.hole_mm
            pitch_disp = result.pitch_mm
        elif unit == "inch":
            hole_disp = result.hole_mm / 25.4
            pitch_disp = result.pitch_mm / 25.4
        else:  # px
        ...
        if (apply_override or self.lens_auto_var.get()) and not self._applying_lens_optimum:
            ...
            try:
                self.hole_var.set(round(hole_disp, 4))
                self.spacing_var.set(round(pitch_disp, 4))
            finally:
